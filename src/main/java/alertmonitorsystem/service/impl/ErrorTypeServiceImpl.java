...
import alertmonitorsystem.utils.GetType;
...
public class ErrorTypeServiceImpl implements ErrorTypeService {
    ...
    @Override
    @CachePut(cacheNames = "addErrorType")
    @Transactional(propagation = Propagation.REQUIRED,isolation = Isolation.READ_COMMITTED)
    public int add(Map<String, Object> map) {
        int result = errorTypeMapper.add(map);
//        System.out.println("id的类型是："+GetType.getType(CastUtil.cast(map.get("id"))));
    ...
    @Override
    @CacheEvict(cacheNames = "delErrorType",allEntries = true,beforeInvocation = true,condition = "#id>0")
    @Transactional(propagation = Propagation.REQUIRED,isolation = Isolation.READ_COMMITTED)
    public int delete(Map<String, Object> map) {

        return errorTypeMapper.delete(map);
    }

    @Override
    @CachePut(cacheNames = "updateErrorType",condition = "#id>0",unless = "#result>0")
    @Transactional(propagation = Propagation.REQUIRED,isolation = Isolation.READ_COMMITTED)
    public int update(Map<String, Object> map) {
        return errorTypeMapper.update(map);
    }

    @Override
    @CachePut(cacheNames = "errorType")
    public Map<String, Object> select(Map<String, Object> map) {

        List<Map<String, Object>> resultList = errorTypeMapper.select(map);
        // 当前页
        map.put("code","200");
    ...
    @Override
    @CachePut(cacheNames = "errorTypes")
    public Map<String, Object> likeSelect(Map<String, Object> map) {
        ...
        // 获取总个数
//        System.out.println("selectCount类型："+GetType.getType(errorTypeMapper.likeSelectCount(map)));
        Integer totalCount = errorTypeMapper.likeSelectCount(map);

        List<Map<String, Object>> resultList = errorTypeMapper.likeSelect(map);
    ...
    @Override
    @Cacheable(cacheNames = "errorTypeCount")
    public int likeSelectCount(Map<String, Object> map) {
        return errorTypeMapper.likeSelectCount(map);
    }
}
